...
import lab.aikibo.learnbillingpbbwebapp.auth.UserAuthentication;
...
public class TokenAuthService {
    ...
    // 5 hours
    private static final long VALIDITY_TIME_MS = 5 * 60 * 60 * 1000;
    ...
    public String createTokenForUser(DatLoginWebapp user) {
        return Jwts.builder()
    ...
    private String toJSON(DatLoginWebapp user) {
        try {
    ...
    private DatLoginWebapp fromJSON(final String userJSON) {
        try {
            return new ObjectMapper().readValue(userJSON, DatLoginWebapp.class);
        } catch (IOException e) {
            throw new IllegalStateException(e);
        }
    }
